...
def get_input(year, day):
  response = session.get(f'https://adventofcode.com/{year}/day/{day}/input', headers={'User-Agent': 'abra python script email: advent@abra.me'})
...
def submit(year, day, level, ans):
  ...
  response = session.post(
      ...
      headers={'User-Agent': 'abra python script email: advent@abra.me'})
  assert response.status_code == 200, response.text
...
def lines_as_digit_matrix(lines):
  return np.vectorize(int)(lines_as_matrix(lines))
...
def nums_from_string(s):
  return amap(int, re.findall(r'\-?\d+', s))


def split_iter_by_start_indices(iterable, indices):
  res = []
  cur = []
  looking_for_index = 0
  
  for i, j in enumerate(iterable):
    if looking_for_index < len(indices) and i == indices[looking_for_index]:
      if cur:
        res.append(cur)
        cur = []
      looking_for_index += 1
...
def split_iter_by_bools(iterable, bools):
  return split_iter_by_start_indices(iterable, np.where(bools)[0])
